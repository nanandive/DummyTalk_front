import {ChevronsLeft, ChevronsRight, ChevronsRightLeft, Plus, X, ChevronLeft, ChevronRight } from "lucide-react";
...
function Header() {
    ...
    const hi = ["1번", "2번", "3번","4번","5번","6번","7번","8번","9번","10번"]
    // const hi = ["1번", "2번"]
    ...
    return (
        <>
            ...
            <div style={logOutModal ? {display:"block"} : {display:"none"}}>
                <LogOut setLogOutModal = { setLogOutModal } />
            </div>
            <div style={createModal ? {display:"block"} : {display:"none"}}>
                <CreateServer setCreateModal = { setCreateModal } />
            </div>

            {/* 서버 세팅 */}
            <div style={settingModal ? {display:"block"} : {display:"none"}}>
            ...
            <header className="text-md font-semibold px-3 flex items-center h-[60px] bg-[#E7DAB8]">
                <div onClick={ () => onClickSetting()} className="w-[200px]">
                    <img
                ...
                <div style={{display:"flex", marginLeft:"470px"}}>
                    <div onClick={ () => onclickServerCreate()} >
                        <Button className="bg-yellow-600" size="icon">
                            <Plus />
                        </Button>
                    </div>
                    <button onClick={() => setPage(pre => pre >0 ? pre -1 : pre)} style={ hi.length > 6  ? {display:"block", margin:"0px 5px 0px 20px"} : {display:"none"}}>
                        <ChevronLeft className={"text-yellow-600"} />
                    </button>
                    <div style={{display:"flex"}}>
                        {slicedData.map( data =>(
                            <div style={{marginLeft:"10px"}}>
                                <Button className="bg-yellow-600 border-2" size="icon">
                                    {data}
                                </Button>
                            </div>
                        ))}
                    </div>
                    <button onClick={ () => setPage( prev => prev + 1) } style={hi.length > 6 && page + 6 != hi.length ? {display:"block", marginLeft:"15px"}: {display:"none"}}>
                        <ChevronRight className={"text-yellow-600"} />
                    </button>
                </div>
                <div style={{cursor:"pointer"}} className="h-8 w-8 md:h-8 md:w-8 mr-2 ml-auto" onClick={() => onClickModal()}>
                    <UserAvatar src={imageUrl}/>
                </div>
                <div style={{margin:"0px 20px 0px 10px"}}>
                    김지수
                </div>
                <div onClick={() => onClickLogOutModal()} style={{marginRight:"20px",  width:"100px", height:"30px", textAlign:"center", paddingTop:"2px" ,border:"1px solid black", borderRadius:"4px", cursor:"pointer"}}>
                    로그아웃
                </div>
            </header>
        </>
...
function CreateServer( {setCreateModal} ){
    ...
    return (
        <div style={{width:"500px", height:"450px", border:"1px solid black", top:"50%", left:"50%", position: "absolute", transform: "translate(-50%, -50%)"}}>
            ...
            <div style={{ textAlign:"center", marginTop:"15px", fontSize:"20px", color:"gray"}}>
            ...
            <input style={{marginLeft:"50px", backgroundColor:"rgb(233,233,233)", width:"400px", height:"25px" }} type={"text"}/>
            <div style={{display:"flex", margin:"20px 0px 0px 50px"}}>
                <div onClick={() => onClickClose()} style={{cursor:"pointer", width:"80px", height:"30px", fontWeight:"bolder", paddingTop:"5px",}}>
                    뒤로가기
                </div>
                <div style={{cursor:"pointer", textAlign:"center", borderRadius:"5px", width:"80px", height:"30px", backgroundColor:"#FFED46",
                    fontWeight:"bolder", paddingTop:"5px", margin:"0px 50px 0px auto"}}>
                    생성
                </div>
            </div>
        </div>
...
function SettingModal( {setSettingModal} ){
    ...
    return (
        <div style={{width:"500px", height:"450px", border:"1px solid black", top:"50%", left:"50%", position: "absolute", transform: "translate(-50%, -50%)"}}>
            ...
            <div style={{ textAlign:"center", marginTop:"15px", fontSize:"20px", color:"gray"}}>
                새롭게 서버에서 사용할 이름과 <br/> 아이콘을 부여해 개성을 드러내보세요
            </div>
            <div style={{ margin:"20px 180px" }}>
                <img src="./image 29.png"></img>
            </div>

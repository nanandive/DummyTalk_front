...
function Chat({ isOpen, setOpen }) {
    ...
    const onSubmit = async () => {
        const result = await fetch(`http://localhost:9999/channel?id=${channelId}`).then(res => res.json())
        ...
        return (
            ...
            <div className="flex w-full flex-col h-full">
                ...
                <div className="flex flex-col h-1/4 relative overflow-hidden px-5 py-2 rounded-lg">
                    ...
                    {/* 메시지 입력란 */}
                    <Textarea
                        className={`w-full h-full resize-none top-3 outline outline-zinc-300 ${isMouseOver ? 'sparkle' : ''}`}
                        maxLength="500"
                    ...
                    <div className="absolute right-[5%] bottom-[10%] ">
                        ...
                        {/* 메시지 전송 버튼 */}
                        <Button className="h-8 bg-sky-600 text-white" onClick={onSubmit}>Send</Button>
                    </div>
                </div>
            </div>
        );
